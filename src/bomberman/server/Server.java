...
public class Server {
	...
	private Server() {
		...
		try {
			...
			this.database = (Database)jaxser.fromXML(new FileInputStream(ShutdownThread.DATABASE_FILE));
			this.highscore = (Highscore)jaxser.fromXML(new FileInputStream(ShutdownThread.HIGHSCORE_FILE));
		} catch (Exception ex) {
